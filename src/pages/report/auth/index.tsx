...
const SynergismPage = (props: any) => {
    ...
    useEffect(() => {
        const loginInfo = localStorage.getItem('loginInfo')
            ...
            : '';
        setSrc(`${environment}/noHeader/IAM?secret=` + loginInfo.secret);
    }, [environment]);
    ...
    const initAction = () => {
        commitGlobalTitle('report2');
        const loginInfo = localStorage.getItem('loginInfo')
            ? JSON.parse(localStorage.getItem('loginInfo'))
            : '';
        // if (loginInfo && loginInfo.secret) {
        //     setSrc(
        //         'http://dev-sjxt.anchu.vip/noHeader/IAM?secret=' +
        //             loginInfo.secret,
        //     );
        // }
        if (REACT_APP_ENV) {
            ...
            if (loginInfo && loginInfo.secret) {
                setSrc(
                    `${environment}/noHeader/IAM?secret=` + loginInfo.secret,
                );
            }
        }
    };
