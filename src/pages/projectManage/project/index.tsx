...
const SynergismPage = (props: any) => {
    ...
    useEffect(() => {
        const loginInfo = localStorage.getItem('loginInfo')
            ...
            : '';
        setSrc(
            `${environment}/loginBridgeForManager?secret=` + loginInfo.secret,
        );
    }, [environment]);
    ...
    const initAction = () => {
        commitGlobalTitle('projectMana2');
        const loginInfo = localStorage.getItem('loginInfo')
            ? JSON.parse(localStorage.getItem('loginInfo'))
            : '';
        // if (loginInfo && loginInfo.secret) {
        //     setSrc(
        //         'http://dev-admini.phxmgl.cn/loginBridgeForManager?secret=' +
        //             loginInfo.secret,
        //     );
        // }
        if (REACT_APP_ENV) {
            ...
            if (loginInfo && loginInfo.secret) {
                setSrc(
                    `${environment}/loginBridgeForManager?secret=` +
                        loginInfo.secret,
                );
            }
        }
